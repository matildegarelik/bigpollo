...
function remplazastr(palabra,caracter){
	console.log('entra a rempalzar con');
	var busca = caracter;
	var re= new RegExp(busca,'g');
	var mifecha = palabra.replace(re,'');
	console.log('rempalza con: '+mifecha);
	return mifecha;
}
...
var App = {
			...
			datetime: function() {
						var e = new Array("Domingo", "Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado"),
									t = new Array("Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"),
									a = new Date,
									i = a.getYear();
						1e3 > i && (i += 1900);
						var s = a.getDay(),
			...
			side: {
						...
						navigation: function() {
									$(".nav-left a").on("touchstart click", function(e) {
												...
												"home" == t || "" == t || null == t ? $(".html.welcome").addClass("visible") : $(".html." + t).addClass("visible"), App.title($(this).text())
												localStorage.marcador = t;
												if(t=='billetera' || t=='actualiza'){
                           consul_billetera();
													//loadbilletera();
												}
												if(t=='rutas'){loadrutas();}
                        if(t=='productos'){productoscate();productos();chequeo_lectura();}
                        if(t=='clientes'){clienteslist();}
                        if(t=='pedido'){listado_pedidos();chequeo_lectura();}
                        if(t=='devoluciones'){productosdevolucion();}
                        if(t=='movimientos'){consul_mov_diario();}
			...
			hyperlinks: function() {
						$(".nav .nav-item").on("click", function(e) {
									...
									if(t=='billetera' || t=='actualiza'){ consul_billetera(); }//loadbilletera();
                  if(t=='rutas'){loadrutas();}
                  if(t=='productos'){productoscate();chequeo_lectura();}
                  if(t=='clientes'){clienteslist();}
                  if(t=='pedido'){listado_pedidos();chequeo_lectura();}
                  if(t=='devoluciones'){productosdevolucion();}
                  if(t=='movimientos'){consul_mov_diario();}
